...
class GPSMain(object):
    ...
    def run(self, config, itr_load=None):
        ...
        Returns: None
        """
        self.target_points = self.agent._hyperparams['target_state']
        self.target_vel = self.agent._hyperparams['target_vel']
        ...
        for num_pos in range(4):
            flag_fail = True
            self.agent.reset_world(num_pos)
            while flag_fail:
                ...
                if flag_suc_peg:
                    ...
                    if num_pos == 3:
                        self.algorithm.policy_opt.update_ewc(train_obs_data, train_mu, train_prc, train_wt, lam_base,
                                                             with_ewc=True, compute_fisher=False)
                    elif num_pos == 0:
                        self.algorithm.policy_opt.update_ewc(train_obs_data, train_mu, train_prc, train_wt, lam_base,
                                                             with_ewc=False, compute_fisher=True)
                    else:
                        self.algorithm.policy_opt.update_ewc(train_obs_data, train_mu, train_prc, train_wt, lam_base,
                                                             with_ewc=True, compute_fisher=True)
                    print("123")
                    for idx in range(4):
                        self.agent.reset_world(idx)
                        for _ in range(4):
                            self._take_policy_samples()


            # self.data_logger.pickle('./position/algorithm_%02d.pkl' % num_pos, copy.copy(self.algorithm))
            # self.algorithm.policy_opt.save_ckpt('./position/model_%02d.ckpt' % num_pos)
    ...
    def test_policy(self, itr, N):
        ...
        Returns: None
        """
        model_file = './position/model_%02d.ckpt' % itr
        self.algorithm.policy_opt.restore_ckpt(model_file)
        for i in range(self._hyperparams['common']['conditions']):
            self.agent.reset_world(i)
            for _ in range(4):
                self._take_policy_samples()
...
def main():
    ...
    if args.targetsetup:
        ...
        gps.test_policy(itr=test_policy_N, N = test_policy_N)
        ...
        #     plt.show()
        # else:
        #     gps.test_policy(itr=test_policy_N)
    else:
        import random
        import numpy as np
        import matplotlib.pyplot as plt
