...
class GPSMain(object):
    ...
    def run(self, itr_load=None):
        ...
        end_idx = self._hyperparams['iterations']
        ...
        for itr in range(itr_start, self._hyperparams['iterations']):
            ...
            if itr == end_idx - 1:
                save_mu, _, save_obs, _ = self.train_prepare(traj_sample_lists)
                save_nn_mu, _, _, _ = self.algorithm.policy_opt.prob(save_obs)
                np.save('./position/ori_save_mu.npy', save_mu)
                np.save('./position/ori_save_nn_mu.npy', save_nn_mu)
