"""
test OLGPS in small area
policy without  alpha
test position is incremental
"""
...
class GPSMain(object):
    ...
    def run(self, config, itr_load=None):
        ...
        position_train = self.data_logger.unpickle('./position/compare_alpha_1/position_train.pkl')
        test_position = self.data_logger.unpickle('./position/compare_alpha_1/position_test.pkl')
        # all_position_train = np.zeros(position_train.shape)
            ...
            while(flag_fail):
                ...
                if not self.algorithm.traj_opt.flag_reset:
                    ...
                    if num_pos >= 2 :
                        ...
                        self.data_logger.pickle('./position/without_alpha_distance_%d.pkl' % (num_pos - 2), ee_distance)
                        self.data_logger.pickle('./position/without_alpha_cost_%d.pkl' % (num_pos - 2), cost)
                        self.data_logger.pickle('./position/without_alpha_suc_%d.pkl' % (num_pos - 2), pos_suc_count)
