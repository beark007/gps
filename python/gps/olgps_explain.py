        ...
        for num_pos in range(position_train.shape[0]):
            ...
            while(flag_fail):
                ...
                if not self.algorithm.traj_opt.flag_reset:
                    ...
                    """ test OLGPS generalization"""
                    if num_pos == position_train.shape[0] - 1:
                        test_position = self.data_logger.unpickle('./position/test_position_1.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.expand_dims(ee_distance, axis=0)
                        test_position = self.data_logger.unpickle('./position/test_position_2.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.vstack((all_ee_distance, ee_distance))
                        test_position = self.data_logger.unpickle('./position/test_position_3.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.vstack((all_ee_distance, ee_distance))
                        test_position = self.data_logger.unpickle('./position/test_position_4.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.vstack((all_ee_distance, ee_distance))
                        test_position = self.data_logger.unpickle('./position/test_position_5.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.vstack((all_ee_distance, ee_distance))
                        test_position = self.data_logger.unpickle('./position/test_position_6.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.vstack((all_ee_distance, ee_distance))
                        test_position = self.data_logger.unpickle('./position/test_position_7.pkl')
                        cost, pos_suc_count, ee_distance = self.test_cost(test_position)
                        all_ee_distance = np.vstack((all_ee_distance, ee_distance))
                        self.data_logger.pickle('./position/ol_all_distance.pkl', all_ee_distance)
