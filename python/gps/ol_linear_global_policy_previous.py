...
train with previous NN outputs with same obs
    should change the copy hyperparam_pre_data.py to hyperparam.py
"""
...
class GPSMain(object):
    ...
    def run(self, config, itr_load=None):
        ...
        for num_pos in range(position_train.shape[0]):
            ...
            previous_mu = None
            if num_pos > 0:
                previous_mu = self.algorithm.get_previous_sample(train_obs_data)
            else:
                previous_mu = train_mu
            # self.algorithm.policy_opt.update(train_obs_data, train_mu, train_prc, train_wt)
            self.algorithm.policy_opt.update(train_obs_data, train_mu, previous_mu, train_prc, train_wt)
