        ...
        for num_pos in range(position_train.shape[0]):
            ...
            while(flag_fail):
                ...
                if not self.algorithm.traj_opt.flag_reset and flag_suc_peg:
                    ...
                    if num_pos == 0:
                        self.algorithm.policy_opt.update_ewc(train_obs_data, train_mu, train_prc, train_wt, with_ewc=False)
                    else:
                        self.algorithm.policy_opt.update_ewc(train_obs_data, train_mu, train_prc, train_wt, with_ewc=True)
